  ...
  return (
    <div className={css.modal} onClick={handleBackdropClick}>
      <div className={css.modalContent}>
        ...
        <div className={css.textModalContainer}>
          <h2 className={css.carItemTitle}>{make} <span className={css.carItemTitleSpan}>{model}</span>, {year}</h2>
          <p className={css.carItemText}>{address.split(",").slice(1, 2)} | {address.split(",").slice(2, 3)} | Year: {year} | Type: {type}</p>
          <p className={css.carItemText}>Fuel Consumption: {fuelConsumption} | Engine Size:{engineSize} | {functionalities.slice(0, 1)}</p>
          <p>{description}</p>
          <h3 className={css.modalTitle}>Accessories and functionalities:</h3>
          <div className={css.functionalities}>
            <p className={css.carItemText}>{functionalities.slice(0, 1)} | {functionalities.slice(1, 2)} | {functionalities.slice(2, 3)}</p>
          </div>
          <h3 className={css.modalTitle}>Rental Conditions: </h3>
          <div>
            <div className={css.conditionsContainer}>
              <p className={css.conditions}>&nbsp; Minimum age : <span className={css.conditionsSpan}>25 &nbsp;</span> </p>
              <p className={css.conditions}>&nbsp; Valid driver’s license &nbsp;</p>
            </div>
            <div className={css.conditionsContainer}>
              <p className={css.conditions}>&nbsp; Security deposite required &nbsp;</p>
              <p className={css.conditions}>&nbsp; Mileage: <span className={css.conditionsSpan}>{Number(mileage).toLocaleString("en-US")}</span>&nbsp; </p>
              <p className={css.conditions}>&nbsp; Price:<span className={css.conditionsSpan}>{rentalPrice}</span>&nbsp;</p>
            </div>
          </div>
